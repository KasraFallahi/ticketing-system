...
function initAuthentication(app, db) {
			...
			db.authUser(email.toLowerCase(), password)
				.then((user) => {
					if (user) done(null, user);
							...
							{ status: 401, msg: 'Incorrect username and/or password' },
							false
						);
				})
				.catch(() =>
					/* db error */ done({ status: 500, msg: 'Database error' }, false)
				);
		})
	);
	...
	app.use(
		session({
			secret:
				'586e60fdeb6f34186ae165a0cea7ee1dfa4105354e8c74610671de0ef9662191',
			resave: false,
